...
function onError(err, req, res) {
	...
	else res.end(err.message || https.STATUS_CODES[code]);
}
...
class Polka extends Trouter {
	...
	listen() {
		(this.server = this.server || https.createServer(options)).on('request', this.attach);
...
server.server.on(
	'request',
	/** @param {import('node:https').IncomingMessage} req */
	(req) => {
		requests++;
